...
**Excel Normalization** from the source repository.
...
Output: `dist\ExcelNormalization\` — a self-contained folder with
`ExcelNormalization.exe` and all required DLLs/assets.

The script automatically:
- Cleans previous `build\` and `dist\` artefacts
- Removes stale `__pycache__` bytecode
- Runs PyInstaller with `ExcelNormalization.spec`
- Verifies that the exe and key assets are present in the output

You can test the bundle immediately without installing:

```bat
dist\ExcelNormalization\ExcelNormalization.exe
```
...
Output: `installer\Output\ExcelNormalization_Setup_1.0.0.exe`

---

## What the installer does

- Installs to `%ProgramFiles%\Excel Normalization\`
- Pre-creates writable runtime directories under `%LOCALAPPDATA%\ExcelNormalization\`
...
```
%LOCALAPPDATA%\ExcelNormalization\
  uploads\
  work\
  output\
  app.log
...
1. Copy `installer\Output\ExcelNormalization_Setup_1.0.0.exe` to the target machine.
...
6. Click **Run Normalization** and confirm corrected columns appear.
7. Click **Export / Download** and confirm a `.xlsx` file downloads.
8. Check `%LOCALAPPDATA%\ExcelNormalization\app.log` for any errors.
9. Uninstall via **Add or Remove Programs** and confirm the entry is gone.
10. Confirm `%LOCALAPPDATA%\ExcelNormalization\` still exists (user data preserved).
...
Run `ExcelNormalization.exe` from a Command Prompt to see the full
traceback, then check `app.log`.

**PyInstaller misses a hidden import**
Add the missing module to `hidden_imports` in `ExcelNormalization.spec`
