...
def _make_source_workbook() -> Workbook:
    ...
    # Create the three VBA source sheets with raw headers that both:
    # - Excel processors can normalize (insert corrected columns)
    # - ExcelToJsonExtractor can detect and extract into JSON
    for name in ["דיירים יחידים", "מתגוררים במשקי בית", "אנשי צוות ובני משפחותיהם"]:
        ...
        # Row 2: date sub-headers under each date group
        ...
        # Birth split columns (year/month/day): cols 7/8/9
        ws.cell(3, 7).value = 1990
        ws.cell(3, 8).value = 5
        ws.cell(3, 9).value = 15

        # Entry date columns are intentionally absent in this synthetic workbook.

        # Invalid/blank row that should be skipped
        ws.cell(4, 1).value = ""
        ws.cell(4, 2).value = ""
        ws.cell(4, 3).value = ""
        ws.cell(4, 4).value = ""
...
def _read_export_table(path: Path) -> dict[str, list[list]]:
    ...
    for name in ["DayarimYahidim", "MeshkeyBayt", "AnasheyTzevet"]:
        ws = wb[name]
        # Normalize output so empty string vs None differences don't fail parity
        # (Excel treats both as blank cells).
        values: list[list] = []
        # Determine width from the header row (row 1)
        last_header_col = 0
        ...
        # Trim trailing completely-empty rows (except header row)
...
def test_export_processor_vs_json_parity(tmp_path: Path):
    # Build a minimal source workbook on disk
