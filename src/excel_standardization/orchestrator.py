"""Orchestrator for coordinating standardization across all worksheets.

This module provides the standardizationOrchestrator class that coordinates
...
class standardizationOrchestrator:
    ...
    def __init__(self) -> None:
        ...
        self.logger.debug("standardizationOrchestrator initialized")
    ...
    def export_raw_json(self, input_excel_path: str, output_json_path: str) -> None:
        ...
        any standardization. This preserves the exact original values from the Excel file.
    ...
    def export_normalized_json(self, input_excel_path: str, output_json_path: str) -> None:
        ...
        Extracts data from Excel workbook, applies standardization engines, and exports
        ...
        try:
            ...
            # Create standardizationPipeline instance reusing engines from __init__
            ...
            self.logger.info(f"standardizing {len(workbook_dataset.sheets)} sheet(s)")
            normalized_sheets = []

            for idx, sheet_dataset in enumerate(workbook_dataset.sheets, start=1):
                self.logger.info(
                    f"standardizing sheet {idx}/{len(workbook_dataset.sheets)}: '{sheet_dataset.sheet_name}'"
                )
                ...
                try:
                    ...
                    stats = normalized_sheet.get_metadata("standardization_statistics", {})
    ...
    def export_vba_parity_workbook_from_json(self, input_excel_path: str, output_excel_path: Optional[str] = None) -> str:
        """Run JSON standardization pipeline then create VBA-parity export workbook.

        Pipeline:
            Excel workbook -> ExcelToJsonExtractor -> standardizationPipeline -> ExportEngine
        """
        from .io_layer.excel_to_json_extractor import ExcelToJsonExtractor
        from .processing.standardization_pipeline import standardizationPipeline
        ...
        try:
            if workbook_dataset is not None:
                ...
                for sheet in workbook_dataset.sheets:
                    ...
                    stats = norm_sheet.get_metadata("standardization_statistics", {})
